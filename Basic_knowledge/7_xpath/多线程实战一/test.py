...
def get_imgurl():
    r = requests.Session().get(url=weburl, headers=headers,timeout=5).text
    print('请求完毕')
    for page in range(maxpage):
        imgurl = etree.HTML(r).xpath('//div/p/img/@src')[page]
...
def makedir(path, num):
    isExists = os.path.exists(os.path.join(r"D:\Basic_spider\Basic_knowledge\0 download\呆梨\\" + num, path))
    if not isExists:
        print('系统中无该文件夹,创建的地址是:', path)
        os.makedirs(os.path.join(r"D:\Basic_spider\Basic_knowledge\0 download\呆梨\\" + num, path))
        os.chdir(os.path.join(r"D:\Basic_spider\Basic_knowledge\0 download\呆梨\\" + num, path))
        return True
    else:
...
if __name__ == '__main__':
    num = input('输入文件夹的数字:')
    makedir(r"D:\Basic_spider\Basic_knowledge\0 download\呆梨\\" + num, num)
